...
BEGIN
  ...
  LOOP
    ...
    -- 2. Check if every table has anon role access removed
    IF (
      SELECT has_table_privilege('anon', 'public.' || _table.tablename, 'select,insert,update,delete')
    ) THEN
      RAISE EXCEPTION 'Table "%" has anon role access.', _table.tablename;
    END IF;

    -- 3. Check that every text column in the current table has a constraint
